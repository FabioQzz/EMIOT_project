...
float Zeq_circuit(float current, float period, float R_series, float R_cycle, float R_transient_S, float C_transient_S, float R_transient_L, float C_transient_L){
    ...
    V_tr_S = (R_transient_S * period) / (period * R_transient_S * C_transient_S) * (current + C_transient_S * V_tr_S_1 / period);
    V_tr_L = (R_transient_L * period) / (period * R_transient_L * C_transient_L) * (current + C_transient_L * V_tr_L_1 / period);
